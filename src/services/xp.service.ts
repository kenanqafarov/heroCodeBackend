...
export class XpService {
   ...
   */
  static async addXp(userId: string, xpAmount: number): Promise<{ user: any; levelUp: boolean } | null> {
    if (xpAmount <= 0) {
   ...
   */
  static async awardMatchWin(userId: string): Promise<{ user: any; levelUp: boolean }> {
   ...
   */
  static getNextLevelProgress(user: any): { currentXp: number; requiredXp: number; progressPercent: number } {
